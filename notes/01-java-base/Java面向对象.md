...
- ### 权限修饰符
  - public: 所有人都可以访问
  - default：默认 (相同类，相同包的不同类)
  - protected：子类中可以访问(其他包中的)，以及相同类，相同包的不同类
  - private：仅在本类中可以访问
  - 注意protected和default：
    - default：当前包下使用
...
- ### 成员内部类的修饰符
  ...
  - abstract, final:
    - 可，但使用场景不多   
  - static:
    - 静态内部类不依赖于外部类的实例，因此可以直接创建其对象，而无需创建外部类对象。
    - 访问方式
      - 方式1：访问静态内部类中的非静态成员
      	- 在外部类中访问静态内部类的非静态成员，需要先创建静态内部类的对象：
        - `外部类名.内部类 对象名 = new 外部类名.内部类();`
        - 然后通过该对象访问非静态成员。
      - 方式2：访问静态内部类中的静态成员
      	- 静态内部类的静态成员可以直接通过类名访问：
        - 外部类名.内部类名.成员;`
      	- 因为静态内部类本质上与普通的静态成员类似，它属于外部类的静态部分，因此可以直接通过类名调用，而不需要实例化外部类或内部类。
      - 注意事项
       - 如果内部类本身被 static 修饰，它无法访问外部类的非静态成员。这个限制与静态方法的访问规则类似。
      - 静态内部类之所以可以直接访问外部类的静态成员，是因为它不依赖于外部类的实例，而是直接持有外部类的类名。
